<template>
  <div class="storeCard">
    <div class="header">
      ...
      <span class="sorting-menu" v-clickOutside="closeEvent">
        ...
        <div class="menu-warpper" v-if="menuVisible">
          <p class="menu-text" @click="sortByFirstLetter()">按卡券首字母排序</p>
    ...
    <div class = "search-panel">
      <el-dialog title="输入卡券名搜索" :visible.sync="dialogVisible" size="large" :before-close="handleClose">
        <el-input class = "search-input" placeholder="请输入卡券名" icon="search" v-model="searchInput" :on-icon-click="handleIconClick"></el-input>

        <div class="card" v-if="searchCardVisible">
          <h2>卡券名称：{{ card[id].name }}</h2>
          <p>面值：{{ card[id].denomination }}</p>
          <el-button type="primary" class = "to-store-button">领取</el-button>
        </div>
